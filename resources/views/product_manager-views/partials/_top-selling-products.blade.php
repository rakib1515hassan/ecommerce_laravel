        ...
        @foreach($top_sell as $key=>$item)
            @if(isset($item->product))
                <div class="col-md-4 col-sm-6 mt-2"
                     onclick="location.href='{{route('product_manager.product.view',[$item['product_id']])}}'"
                     style="cursor: pointer;padding-right: 6px;padding-left: 6px">
                    <div class="grid-card">
                        <label class="label_1">{{translate('Sold')}} : {{$item['count']}}</label>
                        <div class="text-center mt-3">
                            <img style="height: 90px"
                                 src="{{\App\Services\ProductManager::product_image_path('thumbnail')}}/{{$item->product['thumbnail']}}"
