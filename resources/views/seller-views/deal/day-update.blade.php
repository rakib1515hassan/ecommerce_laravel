...
@section('title', translate('Deal Update'))
@push('css_or_js')
    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{translate('Deal of the Day')}}</li>
                    ...
                    <div class="card-body">
                        <form action="{{route('admin.deal.day-update',[$deal['id']])}}"
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <select
                                            ...
                                            @foreach (\App\Models\Product::orderBy('name', 'asc')->get() as $key => $product)
                                                <option
                                                        value="{{ $product->id }}" {{$deal['product_id']==$product->id?'selected':''}}>
                                                    {{$product['name']}}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="card-footer pl-0">
                                <button type="submit"
                                        class="btn btn-primary float-right">{{ translate('update')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
