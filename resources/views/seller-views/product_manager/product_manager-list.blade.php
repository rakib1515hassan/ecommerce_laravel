                    ...
                    <div class="card-body" style="padding: 0">
                        <div class="table-responsive">
                            <table id="datatable"
                                ...
                                <tbody>
                                @foreach ($product_manager as $k => $pm)
                                    <tr>
                                        <th scope="row">{{ $k + 1 }}</th>
                                        <td>{{ $pm->f_name }}</td>
                                        <td>{{ $pm->l_code }}</td>
                                        ...
                                        <td>
                                            <label class="switch">
                                                <input type="checkbox" class="status pm-status" id="{{ $pm['id'] }}"
...
@push('script')
    <script>
        ...
        $(document).on('change', '.pm-status', function () {
            var id = $(this).attr("id");
            ...
            $.ajax({
                url: "{{route('seller.product_manager.updateStatus')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
                ...
                success: function (data) {
                    if (data.success == true) {
                        ...
                        toastr.error('{{translate('Status updated failed. Product Manager must be approved')}}');
                        location.reload();
                    }
                }
            });
