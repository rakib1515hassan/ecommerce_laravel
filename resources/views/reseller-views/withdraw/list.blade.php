                            ...
                            <table id="datatable"
                                ...
                                @foreach($withdraw_requests as $key=>$withdraw_request)
                                    <tr>
                                        ...
                                        <td>
                                            @if($withdraw_request->approved==0)
                                                <button id="{{route('reseller.business-settings.withdraw.cancel', [$withdraw_request['id']])}}"
                                                        onclick="close_request('{{ route('reseller.business-settings.withdraw.cancel', [$withdraw_request['id']]) }}')"
...
@push('script_2')
    <script>
        function status_filter(type) {
            ...
            $.post({
                url: '{{route('reseller.business-settings.withdraw.status-filter')}}',
                data: {
