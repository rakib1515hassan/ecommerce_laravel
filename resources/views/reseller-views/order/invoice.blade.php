    ...
    <table class="content-position">
        <tr>
            ...
            <th style="text-align: right">
                <h1 style="color: #030303; margin-bottom: 0px; font-size: 30px;text-transform: capitalize">{{translate('invoice')}}</h1>
                @if($order['seller_is']!='admin' && $order['seller']->gst != null)
                    <h5 style="color: #030303; margin-bottom: 0px;text-transform: capitalize">{{translate('GST')}}
                        : {{ $order['seller']->gst }}</h5>
                @endif
            </th>
        </tr>
    ...
    <table class="bs-0">
        <tr>
            <th class="bg-primary content-position-y" style="padding-right: 0; height: 44px; text-align: left">
                <div>
                    <span class="h4 inline text-white text-uppercase">{{translate('invoice')}} # </span>
                    <span class="inline">
    ...
    <div class=" content-height content-position-y" style="">
        <table class="customers bs-0">
            ...
            <tr class="for-th">
                ...
                <th class="for-th bg-primary">{{translate('item_description')}}</th>
            ...
            @foreach($order->details as $key=>$details)
                ...
                <tr class="for-tb" style=" border: 1px solid #D8D8D8;margin-top: 5px">
                    ...
                    <td class="for-tb for-th-font-bold">{{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($details['price']))}}</td>
                    <td class="for-tb">{{$details->qty}}</td>
                    <td class="for-tb for-th-font-bold">{{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($subtotal))}}</td>
                </tr>
    ...
    <table>
        <tr>
            <th style="text-align: left; vertical-align: text-top;">
                <h4 style="color: #130505 !important; margin:0px;">{{translate('payment_details')}}</h4>
            ...
            <th style="text-align: right">
                <table style="width: 46%;margin-left:41%; display: inline " class="text-right sm-padding strong bs-0">
                    ...
                    <tr>
                        <th class="gry-color text-left"><b>{{translate('sub_total')}}</b></th>
                        <td>{{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($sub_total))}}</td>

                    </tr>
                    <tr>
                        <th class="gry-color text-left text-uppercase"><b>{{translate('tax')}}</b></th>
                        <td>{{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($total_tax))}}</td>
                    </tr>
                    <tr>
                        <th class="gry-color text-left"><b>{{translate('shipping')}}</b></th>
                        <td>{{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($shipping))}}</td>
                    </tr>
                    <tr>
                        <th class="gry-color text-left"><b>{{translate('coupon_discount')}}</b></th>
                        <td>
                            - {{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($order->discount_amount))}}</td>
                    </tr>
                    <tr class="border-bottom">
                        <th class="gry-color text-left"><b>{{translate('discount_on_product')}}</b></th>
                        <td>
                            - {{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($total_discount_on_product))}}</td>
                    </tr>
                    <tr style="background-color: #2D7BFF">
                        <th class="text-left"><b class="text-white">{{translate('total')}}</b></th>
                        <td class="text-white">{{\App\Services\BackEndHelper::set_symbol(\App\Services\BackEndHelper::usd_to_currency($order->order_amount))}}</td>
                    </tr>
                    </tbody>
                </table>
            </th>
            ...
            <tr>
                <th class="content-position-y bg-primary"
                    ...
                    <div class="text-white" style="padding-top:5px; padding-bottom:2px;"><i
                        ...
                        : {{\App\Models\BusinessSetting::where('type','company_phone')->first()->value}}</div>
                    <div class="text-white" style="padding-top:5px; padding-bottom:2px;"><i
                            class="fa fa-globe text-white"
                            aria-hidden="true"></i> {{translate('website')}} : {{url('/')}}</div>
                    <div class="text-white" style="padding-top:5px; padding-bottom:2px;"><i
                            class="fa fa-envelope text-white"
                            aria-hidden="true"></i> {{translate('email')}} : {{$company_email}}</div>
                </th>
