...
@section('title', translate('Seller Information'))
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"
                    aria-current="page">{{translate('seller_settings')}}</li>
            </ol>
        </nav>

        <!-- Page Heading -->
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <h4 class="mb-0 text-black-50">{{translate('sales')}} {{translate('comission')}} {{translate('Informations')}} </h4>
        </div>
        ...
        <div class="row" style="padding-bottom: 20px">
            ...
            <div class="col-md-6">
                <div class="card-header">
                    <h5>{{translate('Sales Commission')}}</h5>
                </div>
                <div class="card">
                    <div class="card-body" style="padding: 20px">
                        <form action="{{route('admin.business-settings.seller-settings.update-seller-settings')}}"
                              method="post">
                            @csrf
                            <label>{{translate('Default Sales Commission')}} ( % )</label>
            ...
            <div class="col-md-6">
                <div class="card-header">
                    <h5>{{translate('Seller Registration')}}</h5>
                </div>
                <div class="card">
                    <div class="card-body" style="padding: 20px">
                        <form action="{{route('admin.business-settings.seller-settings.update-seller-registration')}}"
                              method="post">
                            @csrf
                            <label>{{translate('Seller Registration on/off')}}</label>
                            <div class="form-check">
                                ...
                                <label
                                    class="form-check-label {{Session::get('direction') === "rtl" ? 'mr-3' : 'ml-3'}}"
                                    for="defaultCheck1">
                                    {{translate('Turn on')}}
                                </label>
                            </div>
