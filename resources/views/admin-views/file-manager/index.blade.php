...
@section('content')
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item text-capitalize"
                    aria-current="page">{{translate('file_manager')}}</li>
            </ol>
        </nav>
        <!-- Page Heading -->
        <div class="d-md-flex_ align-items-center justify-content-between mb-2">
            <div class="row">
                <div class="col-md-8">
                    <h3 class="h3 mb-0 text-capitalize text-black-50">{{translate('file_manager')}}</h3>
                </div>

                <div class="col-md-4">
                    <button type="button" class="btn btn-primary modalTrigger  float-right" data-toggle="modal"
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header py-1">
                        ...
                        @endphp
                        <h5 class="text-capitalize">{{end($pwd)}} <span class="badge badge-soft-dark ml-2"
                                                                        id="itemCount">{{count($data)}}</span></h5>
                        <a class="btn btn-primary" href="{{url()->previous()}}"><i
                                    class="tio-left-arrow"></i>{{translate('back')}}</a>
                    </div>
                                    ...
                                    @if($file['type']=='folder')
                                        <a class="btn"
                                           href="{{route('admin.file-manager.index', base64_encode($file['path']))}}">
                                            <img class="img-thumbnail" src="{{asset('assets/back-end/img/folder.png')}}"
                                                 alt="">
                                            <p>{{Str::limit($file['name'],10)}}</p>
                                        </a>
                                    @elseif($file['type']=='file')
                                        <!-- <a class="btn" href="{{asset('storage/app/'.$file['path'])}}" download> -->
                                        <button class="btn w-100" data-toggle="modal" data-target="#imagemodal{{$key}}"
                                                title="{{$file['name']}}">
                                            <div class="gallary-card">
                                                <img src="{{asset('storage/app/'.$file['path'])}}"
                                                     alt="{{$file['name']}}" style="height:auto;width:100%;">
                                            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title"
                            id="exampleModalLabel">{{translate('upload')}} {{translate('file')}} </h5>
                            ...
                            <div class="form-group">
                                <div class="custom-file">
                                    ...
                                    <label class="custom-file-label"
                                           for="customFileUpload">{{translate('choose')}} {{translate('images')}}</label>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="custom-file">
