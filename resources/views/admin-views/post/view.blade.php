    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{translate('post')}}</li>
            </ol>
        </nav>
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="flex-between justify-content-between align-items-center flex-grow-1">
                            <div>
                                <h5>{{ translate('post')}} <span style="color: red;">({{ $posts->total() }})</span>
                                </h5>
                            </div>
                            ...
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                @foreach($posts as $key=>$post)
                                    <tr>
                                        ...
                                        <td>
                                            <img src="{{ asset('storage/post_image/' . $post['image']) }}" alt="image" id="viewer" height="60"/>
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        data: {id: id},
                        success: function () {
                            toastr.success('{{translate('Post_deleted_Successfully.')}}');
                            location.reload();
                        }
                    });
                }
    ...
    <script>
     ...
     <script>
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                url: "{{route('admin.post.status-update')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
