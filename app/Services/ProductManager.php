...
class ProductManager
{
    ...
    {
        $paginator = Product::active()->with(['reviews'])
            ->where('featured', 1)
    ...
    // Searched Products
    public static function get_searched_products($name, $limit = 10, $offset = 1)
    {
        /*$key = explode(' ', $name);*/
        $key = [$name];

        $paginator = Product::with('brand')->active()->with(['rating'])->where(function ($q) use ($key) {
    ...
    public static function translated_product_search($name, $limit = 10, $offset = 1)
    {
        ...
        $paginator = Product::with('brand')->WhereIn('id', $product_ids)->paginate($limit, ['*'], 'page', $offset);
    ...
    {
        $paginator = Product::with(['reviews'])->active()->where('discount', '!=', 0)->where('discount_type', 'percent')->orderBy('discount', 'desc')->paginate($limit, ['*'], 'page', $offset);
        return [
            'total_size' => $paginator->total(),
            'limit' => (int)$limit,
            'offset' => (int)$offset,
    ...
    // public static function search_products($query, $limit = 10, $offset = 1)
    // {
    //     $paginator = Product::active()->with(['rating'])
