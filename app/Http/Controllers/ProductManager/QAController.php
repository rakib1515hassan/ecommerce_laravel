...
class QAController extends Controller
{
    public function index(Request $request)
    {
        $qas = QuestionAnswering::select('question_answerings.id as id', 'products.name', 'question_answerings.question as question', 'question_answerings.status as status')
            ...
            ->where('products.user_id', auth('product_manager')->user()->id);
        ...
        return view('product_manager-views.qa.index', compact('qas'));
    }

    public function show($id)
    {
        $qa = QuestionAnswering::find($id);
        return view('product_manager-views.qa.show', compact('qa'));
    }
    ...
    public function reply(Request $request, $questionId)
    {
        ...
        $question->answered_by = auth('admin')->user() ? auth('admin')->user()->id : auth('product_manager')->user()->id;
