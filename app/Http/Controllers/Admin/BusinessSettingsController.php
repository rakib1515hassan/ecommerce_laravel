...
class BusinessSettingsController extends Controller
{
    ...
    {
        $contact_us = BusinessSetting::where('type', 'contact_us')->first();

        if (!$contact_us) {
            $contact_us = BusinessSetting::create([
                'type' => 'contact_us',
                'value' => 'Our Contact Information',
            ]);
        }
    ...
    public function contact_usUpdate(Request $data)
    {
        $validatedData = $data->validate([
            'contact_us' => 'required',
        ]);
        BusinessSetting::where('type', 'contact_us')->update(['value' => $data->contact_us]);
    ...
    {
        $future_plans = BusinessSetting::where('type', 'future_plans')->first();

        if (!$future_plans) {
            $future_plans = BusinessSetting::create([
                'type' => 'future_plans',
                'value' => 'Our future plans',
            ]);
        }

        return view('admin-views.business-settings.future-plans', [
            'future_plans' => $future_plans,
        ]);

    }

    public function future_plansUpdate(Request $data)
    {
        $validatedData = $data->validate([
            'future_plans' => 'required',
        ]);
        BusinessSetting::where('type', 'future_plans')->update(['value' => $data->future_plans]);
        Toastr::success('Future plans updated successfully!');
        return back();
    }
    ...
    {
        $return_and_refunds = BusinessSetting::where('type', 'return_and_refunds')->first();
        return view('admin-views.business-settings.return-and-refunds', [
            'return_and_refunds' => $return_and_refunds,
        ]);

    }

    public function return_and_refundsUpdate(Request $data)
    {
        $validatedData = $data->validate([
            'return_and_refunds' => 'required',
        ]);
        $r = BusinessSetting::where('type', 'return_and_refunds')->first();

        if ($r) {
            BusinessSetting::where('type', 'return_and_refunds')->update(['value' => $data->return_and_refunds]);
        } else {
            BusinessSetting::create([
                'type' => 'return_and_refunds',
                'value' => $data->return_and_refunds,
            ]);
        }

        Toastr::success('Return and refunds updated successfully!');
        return back();
    }
    ...
    public function updateInfo(Request $request)
    {
        ...
        if ($request->has('shop_banner')) {
            $imgBanner = ImageManager::update('order/', $imgBanner, 'png', $request->file('shop_banner'));
    ...
    public function shop_banner(Request $request)
    {
        ...
        if (isset($img)) {
            $img = ImageManager::update('order/', $img, 'png', $request->file('image'));
            BusinessSetting::where(['type' => 'shop_banner'])->update([
                'value' => $img,
            ]);
        } else {
            $img = ImageManager::upload('order/', 'png', $request->file('image'));
            DB::table('business_settings')->insert([
                'type' => 'shop_banner',
                'value' => $img,
            ]);
