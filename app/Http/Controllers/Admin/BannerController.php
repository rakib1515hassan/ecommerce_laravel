...
class BannerController extends Controller
{
    function list(Request $request)
    {
        ...
        $banners = $banners->paginate(AdditionalServices::pagination_limit())->appends($query_param);
    ...
    public function status(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($banner->banner_type == 'Popup Banner') {
                $banner->published = $request->status;
                $banner->save();

                Banner::where('banner_type', 'Popup Banner')
                    ->where('id', '!=', $banner->id)
                    ->update(['published' => 0]); // false

                return response()->json($banner->published);

            } else {
                $banner->published = $request->status;
                $banner->save();
                // Return the updated published status as a JSON response
                return response()->json($banner->published);
            }
