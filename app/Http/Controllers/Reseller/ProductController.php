...
class ProductController extends Controller
{
    ...
    {
        // $products = Product::where(['added_by' => 'seller', 'status' => '1', 'user_id' => auth('seller')->id()])->get();
        // dd($products);
        $query = Product::query();
        if($request->ajax()){
            $products = $query->where(['added_by' => 'seller', 'status' => '1', 'user_id' => auth('seller')->id()])->get();
            return response()->json(['products'=>$products]);
        }
        $products = $query->where(['added_by' => 'seller', 'status' => '1', 'user_id' => auth('seller')->id()])->get();
        return view('reseller-views.product.add-new', compact('products'));
    }

    public function status_update(Request $request)
    {
        if ($request['status'] == 0) {
            Product::where(['id' => $request['id'], 'added_by' => 'reseller', 'user_id' => \auth('reseller')->id()])->update([
    ...
    //         $product->reseller_id = Auth::guard('reseller')->id();
    //         $product->save();
    //         $data = [];
    ...
    //         return redirect()->route('reseller.product.list');
    //     }
    // }
    ...
    public function store(Request $request)
    {
        ...
        $product->reseller_id = auth('reseller')->id();
        $product->product_id = $request->product_id;
        // $product->slug = Str::slug($request->Product::find()->name . '-' . $product->id);
        $product->save();

        $frontend_url ="https://shojonsl.com/product/";

        $p = Product::where('id',$request->product_id)->first();

        $generatedLink  = $frontend_url . $p->slug;

        $generatedLink  .= '?ref='.$product->id;
        ...
        //Toastr::success('Product added successfully!');
        // return view('reseller-views.product.link_show', ['link' => $product_url]);
    }
    ...
    function list(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $products = Product::where(['added_by' => 'seller', 'reseller_id' => \auth('reseller')->id()]);
        }
        $products = $products->orderBy('id', 'DESC')->paginate(AdditionalServices::pagination_limit())->appends($query_param);

        return view('reseller-views.product.list', compact('products', 'search'));
    }
    ...
    public function stock_limit_list(Request $request, $type)
    {
        ...
        $search = $request['search'];
        $pro = Product::where(['added_by' => 'seller', 'reseller_id' => \auth('reseller')->id()])
        ...
        return view('reseller-views.product.stock-limit-list', compact('products', 'search', 'request_status', 'sort_oqrderQty'));
    }
    ...
    public function get_variations(Request $request)
    {
        $product = Product::find($request['id']);
        return response()->json([
            'view' => view('reseller-views.product.partials._update_stock', compact('product'))->render()
        ]);
    }
    ...
    // {
    //     $product = Product::withoutGlobalScopes()->with('translations')->get();
    //     $product_category = json_decode($product->category_ids);
    //     $product->colors = json_decode($product->colors);
    //     $categories = Category::where(['parent_id' => 0])->get();
    //     $br = Brand::orderBY('name', 'ASC')->get();
    //     return view('reseller-views.product.edit', compact('categories', 'br', 'product', 'product_category'));
    ...
    public function view($id)
    {
        ...
        return view('reseller-views.product.view', compact('product', 'reviews'));
    }
    ...
    {
        return view('reseller-views.product.bulk-import');
    }
    ...
    {
        $products = Product::where(['added_by' => 'seller', 'user_id' => \auth('reseller')->id()])->get();
        //export from product
        $storage = [];
