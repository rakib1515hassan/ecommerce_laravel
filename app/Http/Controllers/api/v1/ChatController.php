...
class ChatController extends Controller
{
    ...
    {
        try {
            $last_chat = Chatting::with(['seller_info', 'customer', 'order'])->where('user_id', $request->user()->id)
                ->orderBy('created_at', 'DESC')
                ->first();

            if (isset($last_chat)) {

                $chattings = Chatting::with(['seller_info', 'customer', 'order'])->join('shops', 'shops.id', '=', 'chattings.shop_id')
                    ->select('chattings.*', 'shops.name', 'shops.image')
                    ->where('chattings.user_id', $request->user()->id)
                    ->where('shop_id', $last_chat->shop_id)
                    ->get();

                $unique_shops = Chatting::with(['seller_info', 'order'])
                ...
                // $unique_shops = Chatting::with(['seller_info', 'order'])->groupBy('shop_id')->get();
    ...
    {
        try {
            $messages = Chatting::with(['seller_info', 'customer', 'order'])->where('user_id', $request->user()->id)
